        ...
        if (postList.length == 0) {
            ...
            for (var i = 0; i < postList.length; i++) {
                if (postList[i]["by"] == undefined) {
                    addPost(postList[i]["title"], "Anonymous", postList[i]["image"], "post.html?play=" + postList[i]["key"], postList[i]["verified"]);
                } else {
                    addPost(postList[i]["title"], postList[i]["by"].substring(0, 30), postList[i]["byStaff"], postList[i]["image"], "post.html?play=" + postList[i]["key"], postList[i]["verified"]);
                }
            }
        }
    });
